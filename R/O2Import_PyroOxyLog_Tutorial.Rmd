...
O2Data <- O2Data %>%
  ...
  mutate(Filename = str_remove(string = Filename, pattern = "Caloxy")) %>%
  separate(Filename, into = c("Run", "FileDateTime", "Project", "FileID", "CultureID", "Ex_WL", "GrowthLight"), sep = "([\\/\\_\\.])", remove = FALSE) %>%
  mutate(FileDateTime = parse_date_time(as.character(FileDateTime), orders = "ymdHM")) %>%
  separate(FileDateTime, into = c("File_YMD", "File_HM"), sep = "([ ])", remove = FALSE)
...
O2Data %>%
  ggplot() +
  geom_point(aes(x = ETime_s, y = O2_umolL, colour = as.factor(Ex_WL))) +
