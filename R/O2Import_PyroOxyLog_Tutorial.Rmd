...
#a read function using tidyverse::read_delim that skips a fixed number of header rows, and adds columns to the dataframe containing the filename and the file creation date time.
 read_delim_plus <- function(flnm, delimiter, fileencode, header){read.delim(file = flnm, delim = delimiter, escape_double = FALSE, trim_ws = TRUE, skip = header,  locale = locale(encoding = fileencode), col_names = TRUE, col_types = NULL) %>%
...
Reads in O2Data files one after another; skips the "notes" at the beginning of the data and begins to read each file after HeaderRows. Renames columns so there are no duplicates (.name_repair)
```{r read optode data}

test <- read_delim(O2DataFiles[[1]], 
    delim = "\t", escape_double = FALSE, 
    trim_ws = TRUE, skip = 13)
...
testmap <- O2DataFiles %>%
  map_dfr(~read_delim(., delim = "/t", escape_double = FALSE, 
    trim_ws = TRUE, skip = 13))
...
test2map <- O2DataFiles %>%
  map_df(~read_tsv_plus(Flnm = ., HeaderRows = HeaderRows))

head(test2map)

O2Data <- O2DataFiles %>% 
  purrr::map_df(~read_delim_plus(flnm = ., delimiter = Delimiter, fileencode = FileEncode, header = HeaderRows))
