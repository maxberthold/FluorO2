...
#a read function using tidyverse::read_delim that skips a fixed number of header rows, and adds columns to the dataframe containing the filename and the file creation date time.
 read.delim_plus <- function(flnm, fileencode, delimiter, header){read.delim(file = flnm, fileEncoding = fileencode, sep = delimiter,  skip = header, row.names = NULL) %>%
...
O2Data <- O2DataFiles %>% 
  purrr::map_df(~read.delim_plus(flnm = ., fileencode = FileEncode, delimiter = Delimiter , header = HeaderRows)) %>%
   select(
    where(
      ~!all(is.na(.x)) #remove columns of all na
    )
