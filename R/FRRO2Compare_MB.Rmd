...
For each run create a series of nests of O2 concentration data from optode for each PAR level and each TempCont setting.
...
For a given sample [PSII] should be a constant; any change in O2 l~-1~ s~-1~ with increasing PAR should be driven solely by changes in e- PSII~-1~ s~-1~.  If apparent [PSII] changes this indicates issues with measurements, as for example recycling of electrons leaving PSII back to reduction of O2 induced under high light, which lowers measured O2 l~-1~ s~-1~.
...
FRRfData %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = JVPSII_aLHIIminmax, colour = Ex_WL)) +
  #geom_point(aes(x = O2Data$LR_s, y = O2Data$O2_umolL, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(Ex_WL), cols = vars(CultureID)) +
  theme_bw()
...
FRRfData %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = JVPSII_ETRqpOxbo, colour = Ex_WL)) +
  #geom_point(aes(x = O2Data$LR_s, y = O2Data$O2_umolL, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(Ex_WL), cols = vars(CultureID)) +
  theme_bw()
...
FRRfData %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = (ETRqpOxbo * Fomin/Sig_m2psii), colour = Ex_WL)) +
  #geom_point(aes(x = O2Data$LR_s, y = O2Data$O2_umolL, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(Ex_WL), cols = vars(CultureID)) +
  theme_bw()
...
O2Slopes <- O2SlopeData %>%
  ...
  mutate(O2evln_O2ls = ((O2_umolLs - mean(O2_umolLs[1], O2_umolLs[5]))* 6.022e17)) %>%
  # nest() %>%
...
O2Slopes %>%
  ...
  #geom_line(aes(x = ActPARinter, y = O2evln_O2ls_OLD, colour = "black")) +
    scale_colour_manual(values = Colours_nm) +
    facet_grid(cols = vars(CultureID), rows = vars(Ex_WL)) +
    theme_bw()
```
...
O2FRRfMeta %>%
  ...
  filter(ETRqpOxbo > 0) %>%
  ggplot() +
  geom_point(aes(y  = (O2evln_O2ls * 4)/ETRqpOxbo, x = ActPAR, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(strain)) +
  theme_bw()
```
...
O2FRRfMeta590 <- O2FRRfMeta %>%
  ...
  filter(O2evln_O2ls > lag(O2evln_O2ls) & ActPAR > lag(ActPAR)) #%>%
...
O2FRRfMetaDark <- O2FRRfMeta %>%
  filter(ActPAR == 0) 
  
O2FRRfMetaFilter <- rbind(O2FRRfMeta445, O2FRRfMeta590, O2FRRfMetaDark)

```


Estimates of [PSII] should be stable vs. ActPAR because [PSII] does not change appreciably during duration of light response measure.
If estimate of [PSII] change vs. ActPAR it indicates depatures from assumptions in underlying data or estimators.

# Fit regressions of FRRf estimators vs. O2evln
```{r FRRFO2regressions}
O2FRRfMetaNest <-  O2FRRfMetaFilter %>%
  filter(O2evln_O2ls > 0) %>%
  #filter(O2evln_O2ls_ALT > 0) %>%
  filter(JVPSII_aLHIIminmax > 0) %>%
  nest(data = -c(Ex_WL)) %>%
  mutate(
  ...
  # ETRqpOxbo_O2_alt = map(data, possibly(~lm(ETRqpOxbo ~ O2evln_O2ls_ALT, data = .x), otherwise = NULL)),
  # ETRqpOxbo_O2_predict_alt = map(ETRqpOxbo_O2_alt, possibly(augment, otherwise = NULL)),
  # ETRqpOxbo_O2_tidied_alt =  map(ETRqpOxbo_O2_alt, possibly(tidy, otherwise = NULL)),
  # ETRqpOxbo_O2_param_alt = map(ETRqpOxbo_O2_alt, possibly(glance, otherwise = NULL)),
  # JVPSII_aLHIIminmax_O2_alt = map(data, possibly(~lm(JVPSII_aLHIIminmax ~ O2evln_O2ls_ALT, data = .x), otherwise = NULL)),
  # JVPSII_O2_predict_alt = map(JVPSII_aLHIIminmax_O2_alt, possibly(augment, otherwise = NULL)),
  # JVPSII_O2_tidied_alt =  map(JVPSII_aLHIIminmax_O2_alt, possibly(tidy, otherwise = NULL)),
  # JVPSII_O2_param_alt = map(JVPSII_aLHIIminmax_O2_alt, possibly(glance, otherwise = NULL)),
  # JVPSII_ETRqpOxbo_O2_alt = map(data, possibly(~lm(JVPSII_ETRqpOxbo ~ O2evln_O2ls_ALT, data = .x), otherwise = NULL)),
  # JVPSII_ETRqpOxbo_O2_predict_alt = map(JVPSII_ETRqpOxbo_O2_alt, possibly(augment, otherwise = NULL)),
  # JVPSII_ETRqpOxbo_O2_tidied_alt =  map(JVPSII_ETRqpOxbo_O2_alt, possibly(tidy, otherwise = NULL)),
  # JVPSII_ETRqpOxbo_O2_param_alt = map(JVPSII_ETRqpOxbo_O2_alt, possibly(glance, otherwise = NULL))
  )
...
O2FRRfMetaNest %>%
  unnest(JVPSII_ETRqpOxbo_O2_tidied)#, JVPSII_ETRqpOxbo_O2_tidied_alt) 

```


```{r save O2FRRfMeta}
saveRDS(O2FRRfMetaFilter, file.path(DataOut, paste(Project, RunDate, Cuvette, StrainID, "O2FRRfMeta.Rds", sep = "_"), fsep = .Platform$file.sep))
```
