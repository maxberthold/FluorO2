...
FRRfData %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = LR_s, y = JVPSII_aLHIIOxbomax, colour = Ex_WL)) +
  #geom_point(aes(x = O2Data$LR_s, y = O2Data$O2_umolL, colour = Ex_WL)) +
  scale_colour_manual(values = Colours_nm) +
  facet_grid(rows = vars(Ex_WL), cols = vars(CultureID)) +
  theme_bw()
...
O2SlopeData <- left_join(x = O2Data, y = FRRfData, by = c(c("Date" = "ObsDate"), c("AbsTime" = "ObsTime"), "CultureID", "Ex_WL"), suffix = c("O2", "FRRf"), keep = FALSE) %>%
  select(c("Date":"LR_sFRRf", "ActPARCorr", "ActPARCorr_photonsm2s")) #%>%
...
Need to filter for Dark1sinter == 0 before slope estimations or plotting vs. ActPARCorrinter
```{r O2FRRfprelimplot2}
O2SlopeData %>%
  #filter(Dark1sinter == 0) %>%
  ggplot() +
...
Filter out first 30 s of each ActPARCorrinter set to skip 'induction' phase.
...
If estimate of [PSII] change vs. ActPARCorr it indicates depatures from assumptions in underlying data or estimators.
