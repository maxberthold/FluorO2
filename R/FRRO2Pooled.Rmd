...
# To Do
Run workflow on data from Sylwia & Mireille

# Done
Find or redo comparison of achieved PAR within the cuvette +/- TC.
Do Mireille or Sylwia have this data.

Do we need to redo comparison with spectroradiometer?
Lena B. will bring one.

Does water differentially attenuate blue light?

# Introduction
Pool [O2] vs. FRRf data from multiple runs.
Generate calibrations and conversion factors for use with other FRRf runs from comparable data sets.
...
O2FRRfFiles <- str_subset(O2FRRfFiles,"Nest", negate = TRUE)
O2FRRfFiles
```

Read in .Rds manually and check for missing columns
```{r}
...
#1:6
#10:24
#Fix character vs. numeric  in PQP_Size 7, 8, 9 
O2FRRfAll <- str_subset(O2FRRfFiles,"Nest", negate = TRUE)[-c(7,8,9)] %>%
...
O2FRRfPool %>%
  ggplot() +
  geom_point(aes(x = ActPARCorr, y = JVPSII_aLHIIOxbomax, colour = as.factor(TempContO2))) +
  geom_smooth(aes(x = ActPARCorr, y = JVPSII_aLHIIOxbomax, colour = as.factor(TempContO2)), method = "lm") +
  #scale_colour_manual(values = Colours_nm) +
...
O2FRRfPool %>%
  filter(Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_aLHIIOxbomax, colour = as.factor(TempContO2))) +
  geom_smooth(aes(x = O2evln_O2ls, y = JVPSII_aLHIIOxbomax, colour = as.factor(TempContO2)), method = "lm") +
  #scale_colour_manual(values = Colours_nm) +
...
O2FRRfPool %>%
  filter(#Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = ETRCtauav, y = ETRqpOxbo, colour = as.factor(Ex_WL))) +
  geom_smooth(aes(x = ETRCtauav, y = ETRqpOxbo, colour = as.factor(Ex_WL)), method = "lm") +
  #scale_colour_manual(values = Colours_nm) +
  #facet_grid(rows = vars(Ex_WL)) +
  coord_fixed(ratio = 1) +
  theme_bw()
...
O2FRRfPool %>%
  filter(#Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = ETRCtauav, y = ETRGorbo, colour = as.factor(Ex_WL))) +
  geom_smooth(aes(x = ETRCtauav, y = ETRGorbo, colour = as.factor(Ex_WL)), method = "lm") +
  #scale_colour_manual(values = Colours_nm) +
  #facet_grid(rows = vars(Ex_WL)) +
  coord_fixed(ratio = 1) +
  theme_bw()
...
JVPSII_aLHIIOxbomax shows good patterns of linear increase with light, but is not absolutely calibrated and is not normalized to biomass ([Chl]).
...
For calibration of JVPSII using O2 data, trim to avoid light saturation region, particularly top light level with NoTC.
  Already done in FRRO2Compare.Rmd
...
Alternately, tau based estimators may fail under 590 nm, but this seems less plausible b/c tau based estimates are taken from shape of a decay curve, and should not be influenced by fluorescence from outside PSII excited by 590 nm.
  Trimming at FRRO2Compare.Rmd largely eliminates this issue.
...
Doug: Checked on arithmetic for JVPSII estimators; seem correct.
...
O2FRRfPool %>%
  filter(Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav_FoSig, colour = as.factor(TempContO2))) +
  geom_smooth(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav_FoSig, colour = as.factor(TempContO2)), method = "lm") +
  #facet_grid(rows = vars(Ex_WL)) +
  theme_bw()
...
O2FRRfPool %>%
  filter(Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav, colour = as.factor(TempContO2))) +
  geom_smooth(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav, colour = as.factor(TempContO2)), method = "lm") +
  #facet_grid(rows = vars(Ex_WL)) +
  theme_bw()
...
O2FRRfPool %>%
  filter(Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRqpOxbo, colour = as.factor(TempCont))) +
  geom_smooth(aes(x = O2evln_O2ls, y = JVPSII_ETRqpOxbo, colour = as.factor(TempCont)), method = "lm") +
  #facet_grid(rows = vars(Ex_WL)) +
  theme_bw()
...
O2FRRfPool %>%
  filter(Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRGorbo, colour = as.factor(TempCont))) +
  geom_smooth(aes(x = O2evln_O2ls, y = JVPSII_ETRGorbo, colour = as.factor(TempCont)), method = "lm") +
  #facet_grid(rows = vars(Ex_WL)) +
  theme_bw()
