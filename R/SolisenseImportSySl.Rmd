...
SolFitsTrim <- SolFits %>% 
  ...
  separate(Filename, into = c("SubProject", "Project", "RunDateTime", "exp", "CultureID", "Ex_WL", "Exp_Type"), sep = "([\\/\\_])", remove = FALSE) %>%
...
SolFitsTrim <- SolFitsTrim %>%
  group_by(Filename, CultureID, Ex_WL) %>%
...
SolFitsTrim2 <- SolFitsTrim %>%
  group_by(Filename, CultureID, Ex_WL, TempCont) %>%
  mutate(Fodark = Fo[1],
         Fmdark = Fm[1],
         Sigdark = Sig[1],
