...
O2FRRfAll %>%
  filter(#Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav_FoSig, colour = as.factor(Ex_WL), size = ActPARCorr)) +
...
O2FRRfAll %>%
  filter(#Ex_WL == 445,
         ActPARCorr != 0) %>%
  ggplot() +
  geom_point(aes(x = O2evln_O2ls, y = JVPSII_ETRtauav_aLHII_Sig, colour = as.factor(Ex_WL), size = ActPARCorr)) +
