...
# To Do
Corrected values for Excitation; does this 'happen' during the re-fit if the proper calibration file settings are chose, but the recalibration for ActPAR has to be done post-hoc
...
ActPARCrossCal <- readRDS("~/Dropbox/CampbellLabProtocols/ChlorophyllFluorescence/SolisenseInformation/SolisenseInformation_DCCrossParam.Rds")
...
SolFitsTrimTest <- SolFitsTrim |>
  mutate(Light_1Corr = case_when(TempCont == "TC" ~ Light_1 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr1_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_1 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr1_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]),
         Light_2Corr = case_when(TempCont == "TC" ~ Light_2 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr2_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_2 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr2_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]),
         Light_3Corr = case_when(TempCont == "TC" ~ Light_3 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr3_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_3 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr3_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]),
           Light_4Corr = case_when(TempCont == "TC" ~ Light_4 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr4_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_4 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr4_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]),
          Light_5Corr = case_when(TempCont == "TC" ~ Light_5 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr5_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_5 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "Pwr5_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]),
          Light_IRCorr = case_when(TempCont == "TC" ~ Light_6 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "PwrIR_uE" & ActPARCrossCal$Models == "DCWaterJacketlm_tidy"],
                                 TempCont == "noTC" ~ Light_6 * ActPARCrossCal$Slope[ActPARCrossCal$DCLamp == "PwrIR_uE" & ActPARCrossCal$Models == "DCCuvettelm_tidy"]))
...
SolFitsTrim2 <- SolFitsTrim %>%
  ...
  mutate(Fodark = Fo[1],
         ...
         ActPAR_photonsm2s = ActPAR *  photons_umol,
         Ctau1 = 1/(1 + (Sig_m2psii * ActPAR_photonsm2s * (Tau1QA/us_s))),
         Ctau2 = 1/(1 + (Sig_m2psii * ActPAR_photonsm2s * (Tau2QA/us_s))),
         Ctauav = 1/(1 + (Sig_m2psii * ActPAR_photonsm2s * (TauAv/us_s))),
         ...
         JVPSII_aLHIIminmax = ActPAR_photonsm2s * aLHIIminmax * FvFm, #issue with FvFm in cyanobacteria; minimized with blue light excitation
         JVPSII_aLHIIdark = ActPAR_photonsm2s * aLHIIdark * FvFm,
         JVPSII_aLHIIdark = ActPAR_photonsm2s * aLHIIOxbomax * FvFm,
         ETRCtau1 = Sig_m2psii * Ctau1 * ActPAR_photonsm2s,
         ETRCtau2 = Sig_m2psii * Ctau2 * ActPAR_photonsm2s,
         ETRCtauav = Sig_m2psii * Ctauav * ActPAR_photonsm2s,
         ETRqp = Sig_m2psii * qp * ActPAR_photonsm2s,
         ETRqpOxbo = Sig_m2psii * qpOxbo * ActPAR_photonsm2s,
         #ETRGorbo = (1/TauAv[7])*((ActPAR*FvFm)/(ActPAR[7]*FvFm[7])) * us_s,
         TestTauAvSat = 1/TauAv[ActPAR == max(ActPAR)],
         ETRGorbo = (1/TauAv[ActPAR == max(ActPAR)])*((ActPAR*FvFm)/(max(ActPAR))*(FvFm[ActPAR == max(ActPAR)])) * us_s,  #Pmax x scaling for change in yield and light
         #Confirm indexing is working; relies upon phiPSII which is not absolute in cyanos
         JVPSII_ETRtauav = ETRCtauav * Fomin/Sigmax, #Sigmax not converted A2_m2
         JVPSII_ETRqpOxbo = ETRqpOxbo * Fomin/Sigmax,
         JVPSII_ETRtauav = ETRCtauav * aLHIIOxbomax/Sigmax,
         JVPSII_ETRqpOxbo = ETRqpOxbo * aLHIIOxbomax/Sigmax) %>%
  ungroup()
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPAR, y = aLHIIOxbomax, colour = as.factor(TempCont))) +
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = ActPAR, y = ETRqpOxbo, colour = as.factor(TempCont))) +
  geom_point(aes(x = ActPAR, y = ETRGorbo), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  theme_bw()
#ETRGorbo looks fast and is unexpectedly similar between 440 & 590
...
SolFitsTrim2 %>%
  filter(Dark1s == 0) %>%
  ggplot() +
  geom_point(aes(x = Ctauav, y = qp, colour = as.factor(TempCont))) +
  #geom_point(aes(x = ActPAR, y = aLHIIminmax), shape = "cross") +
  #scale_colour_manual(values = Colours_nm) +
  facet_grid(cols = vars(Ex_WL), rows = vars(CultureID)) +
  geom_abline(intercept = 0, slope = 1, linetype = "dashed") + 
