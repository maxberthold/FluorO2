...
Optode Data already merged with MetaData
...
O2SlopeData <- left_join(x = O2Data, y = FRRfDataLight, by = c("Project", "Run", "CultureID", "Ex_WL", "TempCont", "ObsDate","LR_s"), suffix = c("O2", "FRRf"), keep = FALSE)
...
O2SlopeData <- O2SlopeData %>%
  select(-c("PIF","LEDSel", "Tau1PQ","Alp2PQ","Tau2PQ","Alp3PQ",  "exp","Exp_Type","nm445", "nm470","nm505","nm535","nm590","IR"))
```
...
```
Problem if LR_s used for join; lose separate LR_sFRRf used for interpolations.
...
O2Slopes <- O2SlopeData %>%
    nest(O2data = -c(Date, Run, ActPARCorrinter, FilenameO2,CultureID, CultureID, CultureID, Ex_WL, GrowthLight, TempCont, ObsDate)) %>%
